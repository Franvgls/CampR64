...
#' @param layout Organización de gráficos en filas ó columnas c(r,c). NA por defecto usa un layout automático.
...
#' denstall64.camp(1,42,Nsh[25:42],"cant","local",layout=c(3,6))
#' }
#' @export
denstall64.camp <- function(gr, esp, camp, zona="cant", dns=c("local","serv"),
                            cor.time=TRUE, ti=FALSE, bw=TRUE, es=TRUE,
                            plot=TRUE, idi="l", layout=NA, years=TRUE,
                            cexleg=1, verbose=FALSE) {
  options(scipen=2)
  ...
  if (length(esp)>1 | any(esp=="999")) {
    ...
    medida <- ifelse(unid64.camp(gr,esp,zona,dns)[1]==1, "cm", "mm")
  }
  if (bw) {
  ...
  if (length(camp)==1) {
    foo <- lattice::densityplot(~rep(talla, a$n*ylim[1]), a,
             plot.points=FALSE, lwd=2, xlim=xlimi, nint=40,
             scales=list(alternating=FALSE, tck=c(1,0), x=list(tick.number=10)),
             main=tit,
             xlab=list(label=ax[1], cex=1.2),
             ...
             panel=function(x, subscripts, ...) {
               ...
               lattice::panel.abline(v=quantile(x, c(.5), na.rm=TRUE)+.5,
                                     lty=1, col="blue", lwd=1)
               lattice::panel.densityplot(x, ...)
             })
  } else {
    # Layout automático SOLO si el usuario no ha pasado nada
    if (any(is.na(layout))) {
      if (ndat>2 & (ndat/2 - trunc(ndat/2))==0) layout <- c(2, ndat/2)
      else                                       layout <- c(1, ndat)
    }
    foo <- lattice::densityplot(~rep(talla, a$n*ylim[1]) | rep(camp, a$n*ylim[1]), a,
             subscripts=TRUE, xlim=xlimi, plot.points=FALSE, nint=15,
             scales=list(alternating=FALSE, tck=c(1,0), cex=.7,
                         x=list(tick.number=10)),
             main=tit,
             xlab=list(label=ax[1], cex=.9),
             ylab=list(label="density", cex=cexleg*.9),
             layout=layout,
             par.strip.text=list(cex=cexleg*.8, font=2),
             as.table=TRUE,
             panel=function(x, ...) {
               lattice::panel.fill(col="white")
