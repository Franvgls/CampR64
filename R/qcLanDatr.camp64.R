...
qclandatr.camp64<-function(camp,zona="cant",dns="local") {
  ...
  plot(dista_p~prof_l,datcamp,pch=21,bg="yellow",ylim=c(0,max(datcamp$dista_p,na.rm=T)*1.1),ylab="Door spread (m)",xlab=c("Depth (m)"))
  ...
  plot(abert_h~prof_l,datcamp,pch=21,bg="yellow",ylim=c(0,max(datcamp$abert_h,na.rm=T)*1.1),ylab=c("Wing spread (m)"),xlab=("Depth (m)"))
  ...
  plot(abert_v~prof_l,datcamp,pch=21,bg="yellow",ylim=c(0,max(datcamp$abert_v,na.rm=T)*1.01),ylab=c("Vertical opening (m)"),xlab=c("Depth (m)"))
