...
#' @examples
#' qcTalPez64.camp("N25", zona="cant", dns="local", gr=1)
#' @seealso \code{\link{qcTalPez.lan64}}
#' @family Control de calidad
#' @export
qcTalPez64.camp <- function(camp, zona = "cant", dns = c("local","serv"),
                             gr = 1, verbose = FALSE) {
  ...
  for (i in lancamp) {
    qcTalPez64.lan(camp = camp, zona = zona, dns = dns,
                   lan = i, gr = gr, verbose = verbose)
  }
