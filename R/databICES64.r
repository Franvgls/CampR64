...
#' @export
databICES64<-function(gr,esp,camp,zona="cant",dns=c("local","serv"),cor.time=TRUE,Nas=FALSE) {
  ...
  dumb1<-sapply(CV.camp64(gr,esp,camp,zona,dns,cor.time=cor.time,excl.sect=c(3:5),Nas=Nas)$sectores[1:2,],t)
  dumb2<-t(CV.camp64(gr,esp,camp,zona,dns,cor.time=cor.time,excl.sect=c(1:2),Nas=Nas)$total[1:2,])
  dumb3<-t(CV.camp64(gr,esp,camp,zona,dns,cor.time=cor.time,Nas=Nas)$total[1:2,])
  dumb4<-sapply(CV.camp64(gr,esp,camp,zona,dns,cor.time=cor.time,ind="n",excl.sect=c(3:5),Nas=Nas)$sectores[1:2,],t)
  dumb5<-t(CV.camp64(gr,esp,camp,zona,dns,cor.time=cor.time,ind="n",excl.sect=c(1:2),Nas=Nas)$total[1:2,])
