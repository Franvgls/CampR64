...
#' @export
NepFUs.camp64<-function(camp=Nsh[length(Nsh)],zona="cant",plot=TRUE,plotnep=TRUE,es=FALSE,ti=TRUE,ICESlab=TRUE,
  ...
  if (plot) {
    MapNort64(places=places,country=country,ICESlab = ICESlab,ICESrect = ICESrect,ICESrectcol = ICESrectcol,FU=FU)
