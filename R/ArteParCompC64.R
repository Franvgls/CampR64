...
#' @param var Variable a comparar: "calones"/"c" (wing spread), "puertas"/"p" (door spread) o "vertical"/"v" (net opening)
...
ArteParComp64 <- function(camp, zona = "cant", dns = c("local","serv"),
  ...
  zona <- tolower(zona)
  var <- match.arg(var, c("calones","puertas","vertical","c","p","v"))
  var <- switch(var, c="calones", p="puertas", v="vertical", var)
